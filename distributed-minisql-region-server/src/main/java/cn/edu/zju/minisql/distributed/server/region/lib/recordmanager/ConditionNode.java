...
public class ConditionNode {
	...
	public String attrName;//功能描述：在非叶节点中为空。在叶节点中表示运算符左侧涉及的属性的名字。
	String tablename2;//功能描述：在非叶节点中为空。在叶节点中表示运算符右侧涉及的属性属于哪个表。如果运算符右侧为常数时该值为空。
	String attrName2;//功能描述：在非叶节点中为空。在叶节点中表示运算符右侧涉及的属性的名字。如果运算符右侧为常数时该值为空。
...
//	实现原理：根据输入进行赋值
	public ConditionNode(String attrName, Comparison op, String value, boolean constantFlag) {
		this.conjunction="";
		...
		if(constantFlag){
			...
			this.attrName2 =value;
		}
	}

	public ConditionNode(String attrName, String op, String value) {//用于unique key的简单条件
		this.attrName = attrName;
		this.conjunction="";
...
//	实现原理：从根节点开始，递归地进行计算。具体算法详见后文。
	public boolean calc(String tablename, Tuple T) {
		if (conjunction.equals("and"))
			...
			if (op == Comparison.eq) {
				if (CatalogManager.getType(tablename, attrName).equals("int")) {
					int num1 = Integer.parseInt(T.units.elementAt(CatalogManager.getAttriOffest(tablename, attrName)));
					int num2;
					if (constantFlag)
						num2 = Integer.parseInt(value);
					else
						num2 = Integer.parseInt(T.units.elementAt(CatalogManager.getAttriOffest(tablename, attrName2)));
					if (num1 != num2)
						return false;
				}
				if (CatalogManager.getType(tablename, attrName)
						.equals("float")) {
					float num1 = Float.parseFloat(T.units
							.elementAt(CatalogManager.getAttriOffest(tablename, attrName)));
					float num2 ;
					if (constantFlag)
						num2 = Float.parseFloat(value);
					else
						num2 = Float.parseFloat(T.units
								.elementAt(CatalogManager.getAttriOffest(tablename, attrName2)));
					if (num1 != num2)
						return false;
				}
				if (CatalogManager.getType(tablename, attrName).equals("char")) {
					String num1 = T.units.elementAt(CatalogManager.getAttriOffest(tablename, attrName));
					String num2;
					if (constantFlag)
						num2 = value;
					else
						num2 = T.units.elementAt(CatalogManager.getAttriOffest(tablename, attrName2));
					if (!num1.equals(num2))
						return false;
				}
